...
def genOrderIndependentOutput(prefix, parallel_substrings, suffix, model, tokenizer, max_new_tokens=10, is_order_independent=True, reverse_parallel_substrings_order=False, torch_device="cpu", modify_model=True):
    # Tokenize input text
    ...
    inputPromptLen=len(tokenizer.decode(tokAll['input_ids'][0], skip_special_tokens=True))
    
    if not is_order_independent:
        # Run tests with no attention mask nor position_id intervention
        generated=model.generate(tokAll['input_ids'], max_new_tokens=max_new_tokens, return_dict_in_generate=True, output_scores=True,do_sample=False)
    else:
        ...
        attention_mask_2d = get_attention_mask_2d_n_options(tokA, tokParallel, tokD, tokAll).to(torch_device)
    ...
    text=tokenizer.decode(generated.sequences[0], skip_special_tokens=True)[inputPromptLen:]
